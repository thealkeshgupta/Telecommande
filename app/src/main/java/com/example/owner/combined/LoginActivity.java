...
//
        ...
        if (extras != null) {
            ...
            message = (message.substring(6,ml));
//            Toast.makeText(getApplicationContext(),"I REACHED HERE",Toast.LENGTH_SHORT).show();
        }
        ...
        tContactbtn=(TextView) findViewById(R.id.textview_frnd);
        if (res.getCount() != 0) {
        ...
        tContactbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent tContactIntent = new Intent(LoginActivity.this, TrustedCommunication.class);
                startActivity(tContactIntent);
    ...
    public void help()
    {
        ...
        try {
            ...
            dialog.show();
            Toast.makeText(getApplicationContext(),"I AAM HERE",Toast.LENGTH_SHORT).show();
        }
    }
    public void textOfContact()
    {
        String updatedPin="";
        try {
            updatedPin=newpin();
    ...
    private void contacttContact() {
        mydb = controllerdb.getReadableDatabase();
        Cursor cursor = mydb.rawQuery("SELECT * FROM  registeruser",null);
        cursor.moveToFirst();
        tContact=""+(String)cursor.getString(cursor.getColumnIndex("number1"));
        myContact= ""+(String)cursor.getString(cursor.getColumnIndex("verifyNum"));
        tContactPin= ""+(String)cursor.getString(cursor.getColumnIndex("number1Pin"));
    }
